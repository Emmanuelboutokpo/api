...
export const effectuerControle = async (req: Request, res: Response) => {
 ...
 if (!req.user) {
    ...
    // 🎯 Vérification du rôle (seul CONTROLLEUR peut effectuer un contrôle)
    if (req.user.role !== 'CONTROLLEUR') {
      return res.status(403).json({ 
        success: false, 
        message: "Seuls les contrôleurs peuvent effectuer des contrôles" 
      });
    }
